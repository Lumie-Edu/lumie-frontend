...
export function CreateResourceModal({ open, onOpenChange }: CreateResourceModalProps) {
  ...
  const { mutateAsync: createResource, isPending, error } = useCreateResource();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>새 자료를 등록합니다.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="border rounded-lg p-3 space-y-2 max-h-32 overflow-y-auto">
              {academies.map((academy) => (
                <div key={academy.id} className="flex items-center gap-2">
                  <Checkbox
                    id={`resource-academy-${academy.id}`}
                    checked={selectedAcademyIds.includes(academy.id)}
                    onCheckedChange={() => toggleAcademy(academy.id)}
                  />
                  <Label htmlFor={`resource-academy-${academy.id}`} className="cursor-pointer text-sm font-normal">
                    {academy.name}
                  </Label>
                </div>
              ))}
          ...
          <div className="space-y-2">
            <Label htmlFor="resourceContent">내용 *</Label>
            <Textarea
              id="resourceContent"
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="createIsItImportantResource"
              checked={isImportant}
              onCheckedChange={(checked) => setValue('isItImportantAnnouncement', !!checked)}
            />
            <Label htmlFor="createIsItImportantResource" className="cursor-pointer">
              중요 자료 (상단 고정)
            </Label>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button type="submit" disabled={isBusy}>
              {isBusy ? (isUploading ? '파일 업로드 중...' : '등록 중...') : '자료 등록'}
            </Button>
          </div>
        </form>
      </DialogContent>
